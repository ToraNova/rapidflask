...
from wtforms.fields.html5 import EmailField #additional email field u8
...
class LoginForm(FlaskForm): #Last Edit R7
    ...
    username = StringField('Username',\
        validators=[\
        InputRequired(),\
        Length(min=lim.MIN_USERNAME_SIZE,max=lim.MAX_USERNAME_SIZE)])
    password = PasswordField('Password',
        validators=[\
        InputRequired(),\
        Length(min=lim.MIN_PASSWORD_SIZE,max=lim.MAX_PASSWORD_SIZE)])
...
class System_User_RegisterForm(FlaskForm): #Last Edit R7
    username = StringField('Username',\
        ...
        InputRequired(),\
        Length(min=lim.MIN_USERNAME_SIZE,max=lim.MAX_USERNAME_SIZE)])
    emailadr = EmailField('Email Address',\
        validators=[\
        InputRequired(),\
        Email()])
    password = PasswordField('Password',
...
class Configuration_EditForm(FlaskForm):
    config_value = StringField('Config Value',
        validators=[\
        InputRequired(),\
        Length(min=2,max=lim.MAX_CONFIG_VALU_SIZE)])
